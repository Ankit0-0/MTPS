...
int handle_request(int clientSocketId, ParsedRequest *request, char*tempReq) {
    ...
    if(ParsedHeader_get(request, "Host") == NULL) {
        if(ParsedHeader_set(request, "Host", request->host) < 0) {
            prinft("Set Host header key is not working");
        }
    }
...
void *thread_fn(void *socketNew) {
    ...
    if(temp != NULL) {
        ...
        prinf("Data retrived form the cache\n");
        printf("%s\n\n", response);
        ...
        if(ParsedRequest_parse(request, buffer, len) < 0) {
            prinf("Parsing failed\n");
        }
        else {
            bzero(buffer, MAX_BYTES);
            if(!strcmp(request->method, "GET")) {
                if(request->host && request->path && checkHTTPversioin(request->version) == 1) {
                    ...
                    printf("We only support GET method\n");
                }
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    printf("Building on port %\n", port_number);
    int listen_status = list(proxy_socketId, MAX_CLIENTS);
    if(listen_status < 0) {
...
int add_cache_element(char *data, int size, char *url) {
    int temp_lock_val = pthread_mutex_lock(&lock);
    prif("Add Cache Lock Acquired %d\n", temp_lock_val);
